...
const DAILY_STATUS = {
  ...
  headline: "Soft June transition locked. MT extends through ~June 30 (Heather pays, Sway reimburses). Awaiting Mindbody (Michael Calderon) on Stripe migration method.",
  framing: [
    "🟢 June 1 = LEGAL-ONLY transfer. No signage. No GBP name change. No member email. MT keeps running, members notice nothing.",
    "🎯 June 15 = Mindbody system cutover (with Stripe credit-card migration complete). Sway booking goes live on swaywellnessspa.com.",
    "🎯 July 15 = Massage + facial open at both new locations.",
    "🛟 MT bridge through June 30. Worst case extends further if Stripe slips.",
  ],
  pending: [
    {
      label: "Mindbody (Michael Calderon): PGP file vs destination account ID?",
      detail: "Email sent 12:19 PM May 27. Heather's MT rep is holding pending Mindbody's answer. Follow-up Thursday May 28 EOD.",
      owner: "Awaiting Mindbody",
    },
    {
      label: "Mindbody Payments onboarding at RiNo + Central Park",
      detail: "Other Spavia accounts show in payments.mindbody.io — RiNo + CP don't. Gating item. Push Michael on this.",
      owner: "Awaiting Mindbody",
    },
    {
      label: "Heather's Stripe contact: timeline for migration?",
      detail: "Daily check until reply. Need to know how fast MT can export the encrypted card data.",
      owner: "Awaiting Stripe",
    },
    {
      label: "Heather: Stripe bank redirect (or fallback wire arrangement)",
      detail: "Self-serve in Stripe dashboard if she has access. If not (Custom Connect = MT-managed), submit MT support request. Bridge plan: Heather receives June Stripe payouts, wires to Sway at end of bridge.",
      owner: "Heather",
    },
    {
      label: "Heather: add John as OWNER (not Manager) on both GBP listings",
      detail: "Pre-req for name change later (June 15-20). Owner level required to preserve review history. Do tomorrow regardless of signage timing.",
      owner: "Heather",
    },
    {
      label: "Emily: signage date for each location",
      detail: "Drives GBP rename submission (5 business days before signage). Ask this week.",
      owner: "Emily",
    },
    {
      label: "Insurance (GL + workers' comp) effective June 1",
      detail: "Heather + Dad handling. Confirm certificates in hand.",
      owner: "Heather + Dad",
    },
  ],
  decisionsLockedToday: [
    "🛋️ Lounge model FINAL: 75-min slot (85-min MB backend), cap 15 RiNo / 18 CP, two bookable add-ons (infrared + sauna 25-min), open-floor plunge/compression/lounge",
    "💰 Pricing FINAL: $99 Founding (through Aug 31) → $129 Standard. $49 drop-in / 75 min. $25 first-time intro.",
    "🕐 Hours FINAL: Phase 1 Mon closed, Tue-Fri 7am-8pm, Sat-Sun 8am-6pm. Phase 2 expands to Mon 12-8, Tue-Thu 7-9pm.",
    "📅 June 1 = LEGAL ONLY: no signage, no GBP rename, no email, no website 'open' flip. MT runs as-is.",
    "💳 MT extension: Heather pays MT direct, Sway reimburses (1 net wire at end of bridge).",
    "💌 No email June 1 (Email 07 deprecated). Next email moments: signage day → mid-June cutover → July 15 treatments open.",
    "🏢 GBP name change waits for signage day (submit 5 days before). June 1: only update URL + phone + transition post.",
    "📋 Vendor takeover doc created (docs/upswell-conversion/vendor-subscription-takeover.md) — ask Heather for her May card statement.",
    "🎯 Massage + facial launch target: July 15 (was vaguely 'mid-June' or 'early July').",
  ],
  whatsNext: [
    "Thu May 28: Follow up Michael Calderon if no response. Heather/John request Heather's May card statement to start vendor audit.",
    "Fri May 29: Heather tries Stripe bank redirect (self-serve or MT support). GBP owner invite to John.",
    "Weekend May 30-31: Mindbody session-type config (Sway Remedy Lounge, 85-min backend, 15/18 cap, infrared + sauna resources × 3 rotations).",
    "Mon Jun 1: LEGAL-ONLY transition. Update GBP URL + phone + transition post (name stays Upswell). Insurance live. Daily monitoring begins.",
    "Jun 1-14: Stripe migration runs in parallel. MT continues operating. Weekly Heather/John reconciliation check.",
    "Mid-June: Signage decision day. Submit GBP rename 5 business days before each location's signage.",
    "Jun 15: Mindbody cutover. Sway booking live on swaywellnessspa.com. Members keep cards on file.",
    "Jul 15: Massage + facial bookable at both locations.",
  ],
};
...
const PAYMENT_MIGRATION_PLAN = {
  status: "May 27 — Stripe migration awaiting Mindbody method. MT bridges June 1 → June 30. Heather pays MT direct, Sway reimburses. Members never re-add cards.",
  whyChanged:
  ...
  thePath: [
    ...
    {
      step: "5",
      label: "June 1: LEGAL-ONLY transition. MT operates as before.",
      detail: "Sway owns the locations on paper. No signage, no GBP rename, no website 'open' flip, no email to members. Members book + pay through MT as always. Insurance live. John gets GBP owner access.",
    },
    {
      step: "6",
      label: "June 1-14: Stripe migration runs in parallel with MT operations",
      detail: "MT exports encrypted card data (PGP file or direct Connect transfer). Stripe processes import to Mindbody Connect accounts. Standard timeline: 10 business days from data receipt. Members notice nothing.",
    },
    {
      step: "7",
      label: "~June 10-14: Verify migration complete, test cards in Mindbody",
      detail: "Confirm member cards are visible in Mindbody Payments. Run 3-5 test charges against real member cards (small + refunded) to verify token validity end-to-end. Send pre-cutover heads-up email.",
    },
    {
      step: "8",
      label: "June 15: Mindbody cutover. Sway booking goes live.",
      detail: "Mindbody becomes the POS. Sway Remedy Lounge bookable on swaywellnessspa.com (clone of Larimer pattern). MT booking links removed. Members keep cards on file (no re-add). MT stays as backstop through June 30.",
    },
    {
      step: "9",
      label: "June 15-30: Bridge buffer + Heather/Sway reconciliation",
      detail: "MT remains live as safety net. Mindbody runs primary. Heather sends final wire to Sway covering June Stripe revenue minus reimbursable costs. MT shuts down June 30.",
    },
  ],
  revenueImpact: [
    { window: "June 1 → June 14", expectedPctOfMonthly: 100, note: "MT continues collecting. Stripe routes to Heather's bank during MT processing of redirect. Heather wires Sway end of bridge." },
    ...
    { window: "June 16-30", expectedPctOfMonthly: 100, note: "Mindbody is primary. MT bridge runs as safety net. Reconciliation wire completes." },
    { window: "July 1+", expectedPctOfMonthly: 100, note: "Mindbody-only. MT shutdown. Standard operations." },
  ],
  risks: [
    "🟡 Stripe migration timeline slip: push cutover later. MT extension is on the table per Heather.",
    "🟡 Mindbody Payments onboarding at RiNo + CP not confirmed (per payments.mindbody.io screenshot). Push Michael Calderon.",
    "🟡 Heather may have lost Stripe Connect login. Recover NOW.",
    "🟡 Bank redirect timing: if MT-managed, processing takes days. Accept it. June 1-? revenue lands in Heather's account, wires to Sway at end of bridge.",
    "🟢 Member experience risk = LOW. Cards transfer silently. No re-add friction.",
    "🟢 Heather is cooperative — reconciliation arrangement is low-risk operationally.",
  ],
  reconciliationFlow: [
    "Heather pays MT extension fee from her card on file (existing setup, simplest path)",
    "June Stripe revenue at RiNo + CP flows to Heather's bank during MT processing of bank redirect",
    "Track via Stripe transactions report (Heather has access)",
    "End of bridge (~June 30): single net wire Heather → Sway",
    "Net = (June Stripe revenue at Sway locations) - (MT extension fee) - (other Sway-cost vendor charges Heather covered)",
    "Likely ~$11-14K wire to Sway based on $15K/mo membership base",
  ],
  asks: [
    "🔥 Michael Calderon @ Mindbody: PGP file or destination account ID? RiNo + CP Stripe Connect account IDs? Confirm RiNo + CP onboarded in payments.mindbody.io. (Email sent 12:19 PM May 27.)",
    "Heather: try Stripe direct bank change. If not self-serve, submit MT support request. Either path is fine.",
    "Heather: send John her May card statement for vendor audit (see vendor-subscription-takeover.md).",
    "John + Heather: short email this week documenting reconciliation arrangement (informal, family-business level).",
  ...
  oldPlanArchived:
    "PREVIOUS PLAN (deprecated May 27): Members re-add card at front desk on first Sway visit after a June 1 hard cut. Killed because (a) Stripe migration is feasible, (b) cut-card approach would have lost 20-40% of members to re-add friction, (c) MT bridge with Heather pays-then-Sway reimburses preserves continuity. Earlier plan also assumed June 1 = brand flip with signage + email + GBP rename — now deferred to mid-June (signage timing-dependent).",
};
  ...
  phases: [
    {
      ...
      label: "Legal-only transition. MT continues as POS.",
      items: [
        "Sway owns the locations on paper, no visible change yet",
        "Members book + pay through Mariana Tek as always",
        "Stripe migration runs in parallel",
        "Sway website location pages live with MT booking links",
        "No signage swap yet, no GBP rename yet",
      ],
    },
    {
      phase: "Phase 1 (June 15 → July 14)",
      label: "Mindbody cutover. Recovery lounge bookable on Sway.",
      items: [
        "Sway Remedy Lounge live in Mindbody at both locations",
        "Membership + drop-in pricing live ($99 Founding / $49 drop-in)",
        "Booking on swaywellnessspa.com (Mindbody-powered)",
        "Online drop-ins ON",
        "Signage going up gradually, GBP renames submitted",
        "Hours: Mon closed, Tue-Fri 7-8, Sat-Sun 8-6",
        "MT remains as backstop through June 30 then shutdown",
      ],
    },
    {
      phase: "Phase 2 (July 15+)",
      label: "Massage + facial open.",
      items: [
        "Facial + massage bookable in Mindbody",
        "Hours expand: Mon 12-8, Tue-Thu 7-9, Fri 7-8",
        "Member discount applies (15% off treatments)",
        "Pre-bookings open ~July 5 to fill the schedule",
...
const WEEK_PLAN = [
  {
    ...
    items: [
      "✅ Lounge model + pricing + hours FINAL (see Lounge Decision tab)",
      "✅ Stripe migration path researched + confirmed feasible (Mindbody = Stripe Connect)",
      "✅ Email to Michael Calderon @ Mindbody (12:19 PM)",
      "✅ June plan softened: legal-only June 1, no email, no signage, MT bridges June 1-30",
      "✅ Vendor takeover doc created (docs/upswell-conversion/vendor-subscription-takeover.md)",
      "Heather: confirm Stripe Connect dashboard login access",
      "John: confirm Sway entity bank account ready for payouts",
    ],
  },
  {
    day: "Thu May 28",
    items: [
      "🔥 Follow up Michael Calderon if no response by EOD",
      "Heather: try Stripe direct bank change. If not self-serve, submit MT support request.",
      "Heather: send GBP owner invites to John for both listings",
      "Heather: send John her May card statement (vendor audit starting point)",
      "John: pull final MT data snapshots for Customer Frequency + Reservations (if not done)",
      "John: ask Emily for target signage dates per location",
    ],
  },
  {
    day: "Fri May 29",
    items: [
      "If Mindbody responded: pass method to Heather → MT, kick off data transfer",
      "John: build location pages on swaywellnessspa.com w/ MT booking links + Phase 1 hours",
      "John: walk through Heather's card statement, populate vendor takeover doc",
      "Document Heather-Sway reconciliation arrangement via email",
    ],
  },
  {
    day: "Sat-Sun May 30-31",
    items: [
      "Configure Sway Remedy Lounge session type at both Mindbody sites (85-min backend, cap 15/18, infrared + sauna resources × 3 rotations)",
      "Archive legacy 40-min Remedy Room session at the new locations",
      "Configure Sway Unlimited Founding ($99) + Standard ($129) Mindbody contracts",
      "Configure drop-in product ($49 / 75 min) and first-time intro ($25)",
      "Verify insurance certificates in hand for both locations",
    ],
  },
  {
    day: "Mon Jun 1 — LEGAL TRANSFER ONLY",
    items: [
      "❌ NO email send (deprecated). NO signage. NO GBP name change. NO website 'open' flip.",
      "✅ APA legally closes — Sway owns both locations",
      "✅ Insurance policies effective midnight",
      "✅ John accepts GBP owner invites (Heather sent prior week)",
      "✅ Update GBP: website URL → swaywellnessspa.com/locations/..., phone if transferred, transition post",
      "✅ Location pages on swaywellnessspa.com live (MT booking links, 'coming soon' for treatments)",
      "✅ Quiet staff briefing (Jocelyn handles) — operations continue as Upswell",
      "✅ Monitor: MT running normally, no marketing automations firing, Stripe accounts intact",
    ],
  },
  {
    day: "Tue-Fri Jun 2-12",
    items: [
      "Stripe migration in parallel: MT exports encrypted card data → Stripe imports to Mindbody (10 business days target)",
      "Weekly Heather/John reconciliation: what billed her card, what Stripe collected for Sway",
      "Mindbody Payments verification at RiNo + CP (if not yet onboarded, escalate)",
      "Vendor takeover audit: transfer accounts one by one, document in tracker doc",
      "Get signage timing confirmed from Emily → schedule GBP rename submission (T-5 business days)",
    ],
  },
  {
    day: "~Jun 10-14",
    items: [
      ...
      "Run 3-5 test charges against real member cards (small + refunded) to verify tokens",
      "Front desk training (Jocelyn / Mackenzie): full Mindbody flow",
      "Send pre-cutover heads-up email: 'New booking system live June 15, your card is already on file'",
      "If signage going up mid-June: submit GBP rename now (5 business days lead)",
    ],
  },
  {
    day: "Mon Jun 15 — SYSTEM CUTOVER",
    items: [
      "🔥 Mindbody becomes the POS for both new locations",
      "Sway Remedy Lounge booking goes live on swaywellnessspa.com (clone of Larimer pattern)",
      "Location pages flip from MT booking links → Mindbody booking",
      "Members keep cards on file (Stripe-migrated, no re-add required)",
      "Send launch email: 'New system, new booking. Book your 75-min Sway Remedy Lounge slot here.'",
      "MT remains live as backstop through June 30",
    ],
  },
  {
    day: "Jun 15-30",
    items: [
      "Bridge buffer period — MT live as safety net, Mindbody active",
      "Catch billing edge cases, smooth Mindbody friction",
      "Load massage + facial schedules into Mindbody for July 15 launch",
      "Open pre-bookings for treatments ~July 5",
      "End of June: final MT data snapshot, then MT shutdown",
    ],
  },
  {
    day: "Tue Jul 15 — TREATMENTS LAUNCH",
    items: [
      "Massage + facial bookable at both new locations",
      "Phase 2 hours active (Tue-Thu close 9pm)",
      "Send treatment launch email + social push",
      "First wave of treatment-side acquisition begins",
    ],
  },
];

const BLOCKERS_P0 = [
  "🔥 AWAITING: Michael Calderon @ Mindbody — PGP file vs destination account ID? Confirm RiNo + CP onboarded in payments.mindbody.io. Email sent 12:19 PM May 27. Follow-up Thursday EOD.",
  "🔥 Heather: try Stripe direct bank change. If MT-managed (Custom Connect), submit MT support request. Either path acceptable — bridge plan covers either outcome.",
  "🔥 Heather: add John as OWNER on both GBP listings tomorrow (before June 1). Required for eventual name change.",
  "🔥 Emily: target signage date per location → drives GBP rename submission (T-5 business days).",
  "Insurance (GL + workers' comp) — required effective midnight May 31. Heather + Dad handling. Confirm certificates in hand.",
  "Heather: send John May card statement → starts vendor takeover audit (see vendor-subscription-takeover.md).",
  "Sway Remedy Lounge session-type setup in Mindbody at both sites (85-min backend, cap 15/18, infrared + sauna resources × 3 rotations). Weekend May 30-31.",
  "Update /membership page to show Sway Unlimited tier (Founding $99 / Standard $129 split).",
  "Build /book routes for new locations on swaywellnessspa.com — links to MT through June 14, flips to Mindbody June 15.",
...
const DOCS = [
  ...
  { title: "🆕 Vendor + subscription takeover tracker", path: "docs/upswell-conversion/vendor-subscription-takeover.md" },
  { title: "🆕 Mariana Tek data exports log", path: "docs/upswell-conversion/mariana-tek-data-exports-log.md" },
...
const EMAILS = [
  ...
  { n: "07", date: "❌ DEPRECATED", title: "Was: launch-day welcome. Decision May 27: NO email June 1 (legal-only, nothing visible changes). Repurpose for ~June 14 pre-cutover heads-up.", to: "Active members · 155", path: "email-07-june01-member-welcome.md" },
