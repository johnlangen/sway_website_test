...
const DAILY_STATUS = {
  ...
  pending: [
    ...
    {
      label: "Heather: update Upswell.com Squarespace 'Book' button → Sway booking URLs",
      detail: "Booking URLs DELIVERED June 1. Global 'Book Now' → /clubs/book (new noindex chooser, two club cards, hard-nav so MT widget renders). Per-location pages + GBP → /locations/denver-rino/book + /denver-central-park/book (apex, trailing slash). Heather confirmed she has the per-location booking pages and will update Squarespace + social links when ready. Both work independently if she's slow.",
      owner: "Heather (URLs delivered)",
    },
    {
      label: "GBP: publish transition post on both + add Emily as Manager",
      detail: "Owner ACCEPTED on both listings (john@spaviadayspa.com), June 1. DONE today: Website links → Sway location pages, descriptions rewritten recovery-only (yoga/movement removed), 'Yoga studio' category removed (kept Wellness center + Sauna), socials left as Upswell. REMAINING: publish soft 'new ownership, same recovery space' post on each; add Emily as Manager on both (may hit Google's new-owner cooldown — retry next day); restore CP name qualifier 'Upswell Studio Central Park' if it auto-changed.",
      owner: "John",
    },
    {
      label: "Emily: confirm signage date(s) for each location",
      detail: "Currently ~July 1 (± a week). Confirm — drives GBP rename submission (T-5 business days).",
      owner: "Emily",
    },
    {
      label: "Heather: list of recurring charges on her card we'll take over",
      detail: "Light-touch ask (not full statement). Kicks off vendor takeover audit (docs/upswell-conversion/vendor-subscription-takeover.md).",
      owner: "Heather",
    },
    {
      label: "Account + utilities transfers — EOD today",
      detail: "Heather working with Emily and Jocelyn. Electric, water, internet, phone, trash at both locations.",
      owner: "Heather + Emily + Jocelyn",
    },
    {
      label: "Ops walkthrough Tuesday June 2",
      detail: "Mackenzie + Jocelyn. John joins if possible for context.",
      owner: "Heather + Mackenzie + Jocelyn",
    },
  ],
  decisionsLockedToday: [
    ...
    "✅ DONE: Location pages + /book sub-routes built. MT widget embed code in hand. Ready to go live with embedded MT widget at swaywellnessspa.com/locations/denver-rino and /denver-central-park.",
    "✅ DONE (June 1): GBP owner accepted on BOTH listings. Website links → Sway location pages, descriptions rewritten recovery-only (yoga discontinued, all movement classes removed permanently), 'Yoga studio' category removed (kept Wellness center primary + Sauna), socials LEFT as Upswell until signage. RiNo = 4 recovery sessions; CP = 5 (adds Soak). Don't add massage/day-spa categories until massage launches.",
    "✅ DONE (June 1): Built /clubs/book — noindex club chooser for the location-agnostic 'Book Now'. Isolated from the global /book hub (no saved-location auto-redirect, no shared state) so it can't bounce club guests into Larimer. Two per-location /book URLs are canonical and survive the Mindbody cutover (widget swap, same URLs).",
    "📐 Class model LOCKED (June 1): classes stay the SAME (inherited recovery session types via Mariana Tek) until Mindbody is onboarded. At cutover they collapse into ONE 'Sway Remedy Lounge' that includes everything.",
...
const PAYMENT_MIGRATION_PLAN = {
  ...
  thePath: [
    ...
    {
      step: "5",
      label: "TODAY (June 1): LEGAL-ONLY transition. MT operates as before.",
      detail: "Sway owns the locations on paper. APA closed. Account + utilities transfers in motion via Heather + Emily + Jocelyn. Members book + pay through MT as always. Insurance live. John gets GBP owner access. NO signage, GBP rename, email, or website 'open' flip today.",
    },
    {
      step: "6",
      label: "June 1-30: Stripe migration in parallel + vendor takeover + Mindbody buildout",
      detail: "MT exports encrypted card data (PGP file or destination account ID). Stripe imports to Mindbody Connect accounts. Vendor accounts transferred one by one. Mindbody session types + Lounge product configured. Sway location pages built with embedded MT widget. Members notice nothing.",
    },
    {
      step: "7",
      label: "Mid-to-late June: Verify migration complete, prep for unified launch",
      detail: "Confirm member cards visible in Mindbody Payments. Test charges on real cards. GBP rename submitted T-5 business days before signage. Mindbody internally cutover for staff training; MT still public-facing.",
    },
    {
      step: "8",
  ...
  phases: [
    ...
    },
    {
      ...
      items: [
        ...
        "Heavy acquisition phase begins",
      ],
    },
    {
      phase: "Phase 4 (TBD)",
...
const WEEK_PLAN = [
  {
    day: "Mon Jun 1 (TODAY) — LEGAL TRANSFER",
    items: [
      "✅ APA closes — Sway owns both locations on paper",
      "✅ Insurance policies effective midnight",
      "✅ Account + utilities transfers in motion via Heather + Emily + Jocelyn (EOD today)",
      "✅ Location pages + /book pages built on swaywellnessspa.com with MT widget embedded",
      "✅ Mindbody data team engaged via Katie — no longer waiting on Michael Calderon",
      "John: accept GBP owner invites when Heather sends (she said EOD today)",
      "John: update GBP website URL → swaywellnessspa.com/locations/... + phone + transition post (NAME stays Upswell)",
      "John: GBP transition post on both listings once owner access live",
      "Heather: send 'last chance' email from her voice (MT maintenance context — final Heather-voice send)",
      "WAITING ON: MT migration test file (Heather hounding her rep), GBP owner invites (Heather EOD), Heather's Stripe team response on backend hookup",
      "LOW-PRIO ask: Heather updates Upswell.com Squarespace 'Book' → Sway pages (nice-to-have, not blocker)",
      "NO public messaging. NO signage. NO GBP name change. Members notice nothing.",
    ],
  },
  {
    day: "Tue Jun 2 — Ops walkthrough",
    items: [
      "Heather + Mackenzie + Jocelyn ops walkthrough at both locations",
      "John joins if possible for context",
      "Document edge cases + institutional knowledge",
      "Continue MT widget embed work on Sway location pages",
    ],
  },
  {
    day: "Wed-Fri Jun 3-5",
    items: [
      "Stripe migration kicks off when Mindbody confirms method (PGP or destination account ID)",
      "John: build /locations/denver-rino/book and /locations/denver-central-park/book with embedded MT widget (per coding brief)",
      "Update location landing pages with 'formerly Upswell' framing + Phase 1 hours + tease copy",
      "Heather: send list of recurring vendor charges → vendor takeover audit kickoff",
      "Document Heather-Sway reconciliation arrangement via email (informal)",
    ],
  },
  {
  ...
  {
    day: "Mid-to-late July — TREATMENTS LAUNCH",
    items: [
...
const BLOCKERS_P0 = [
  ...
  "🟡 LOW-PRIO: Heather updates Upswell.com Squarespace 'Book' button → Sway location pages. Both URLs work independently if not done. Ask once.",
  ...
  "🔥 Account + utilities transfers — EOD today via Heather + Emily + Jocelyn.",
  "Sway Remedy Lounge session-type setup in Mindbody at both sites (85-min backend, cap 15/18, infrared + sauna resources × 3 rotations). June first two weeks.",
  "✅ DONE: /locations/denver-rino/book + /locations/denver-central-park/book built with embedded MT widget.",
...
const MY_LIST = [
  ...
  },
  {
    ...
    tasks: [
      ...
      { task: "MEMBER IMPORT (deprecated — Stripe migration handles this): 159 members migrate with cards intact via Stripe PAN import to Mindbody. No manual import needed if migration succeeds.", status: "deprecated", dep: "Stripe migration complete" },
