...
const FACIAL_BOOSTS: Boost[] = [
  ...
  { id: 22, name: "Dermaflash Boost", type: "boost", addsMinutes: 0, memberPrice: "$10", dropInPrice: "$20", description: "Exfoliates and removes peach fuzz.", resourceId: 5 },
  ...
  { id: 19, name: "Sculpt & Lift Microcurrent Pro", type: "boost_pro", addsMinutes: 20, memberPrice: "$25", dropInPrice: "$50", description: "Full-face microcurrent sculpting.", resourceId: 7 },
];

const MASSAGE_BOOSTS: Boost[] = [
  // Boost — $10 member / $20 drop-in, no time added
  { id: 113, name: "CauseMedic CBD Boost", type: "boost", addsMinutes: 0, memberPrice: "$10", dropInPrice: "$20", description: "CBD-infused treatment for deep relaxation and recovery." },
  { id: 26, name: "Cupping Boost", type: "boost", addsMinutes: 0, memberPrice: "$10", dropInPrice: "$20", description: "Suction therapy for deep tension release." },
  ...
  { id: 114, name: "CauseMedic CBD Boost Plus", type: "boost_plus", addsMinutes: 10, memberPrice: "$20", dropInPrice: "$40", description: "Extended CBD-infused treatment for deeper recovery." },
  { id: 115, name: "Cupping Boost Plus", type: "boost_plus", addsMinutes: 10, memberPrice: "$20", dropInPrice: "$40", description: "Extended cupping therapy for full-body tension release." },
];
...
export default function NewBookingFlow() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/service/availability?sessionTypeId=${selectedTreatment.id}&date=${selectedDate}`)
      ...
      .then((data) => {
        ...
        for (const s of data.slots ?? []) { if (s.staffId && s.staffName && !seen.has(s.staffId)) seen.set(s.staffId, s.staffName); }
  ...
  const displayedSlots = (() => {
    ...
    if (req.length > 0) s = s.filter((sl) => req.every((r) => sl.availableResourceIds.includes(r)));
    return s;
  })();
  ...
  const handleFinalConfirmAndBook = async () => {
    ...
    bookingLock.current = true; setStep("booking"); setError(null);
    try {
      ...
      <div className={`px-4 pb-20 ${step === "welcome" ? "pt-16 md:pt-20" : "pt-24 md:pt-28"}`}>
        ...
        {step === "treatment" && (() => {
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            {/* Hero — matches book-service pattern */}
        ...
        {step === "time" && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            {/* Slots */}
            {loading ? <div className="text-center py-12 text-[#113D33]/50">Loading availability...</div>
                ...
                {selectedTreatment && selectedSlot && (
                  <div className="bg-white/80 backdrop-blur-sm border border-[#113D33]/10 rounded-2xl p-4 flex items-center gap-3">
                    <div className="relative w-14 h-14 rounded-xl overflow-hidden shrink-0">
                      <Image src={selectedTreatment.image} alt={selectedTreatment.name} fill className="object-cover" sizes="56px" />
                    </div>
                    <div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="bg-white border border-[#113D33]/10 rounded-2xl overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
              ...
              <div className="relative h-44 w-full overflow-hidden">
                <Image src={selectedTreatment.image} alt={selectedTreatment.name} fill className="object-cover" sizes="100vw" />
        ...
        {step === "done" && selectedTreatment && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center pt-4 space-y-6">
            ...
            <div className="pt-6 space-y-3">
              ...
              <Link href="/locations/denver-larimer/book-new" className={secondaryBtn + " inline-flex items-center justify-center"}>Book Another Massage or Facial</Link>
            </div>
